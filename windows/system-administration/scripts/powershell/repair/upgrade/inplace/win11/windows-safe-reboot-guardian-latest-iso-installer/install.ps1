...
.DESCRIPTION
  Fully automatic per-user installer. Copies the toolkit to LocalAppData, creates launcher commands, and verifies the installed script.
  This installer does not reboot, reset, wipe data, disable security, or require Administrator/UAC by default.
#>
[CmdletBinding()]
param(
    [switch]$Install,
...
$StartMenuDir = Join-Path $env:APPDATA 'Microsoft\Windows\Start Menu\Programs\Windows Safe Reboot Guardian'
function Assert-Source { if (-not (Test-Path -LiteralPath $SourceFix -PathType Leaf)) { throw "Missing source fix.ps1 beside installer: $SourceFix" } }
function Test-FixParse([string]$Path) {
    $tokens = $null; $errors = $null
    [void][System.Management.Automation.Language.Parser]::ParseFile($Path, [ref]$tokens, [ref]$errors)
    if ($errors -and $errors.Count -gt 0) { $messages = ($errors | ForEach-Object { $_.Message }) -join '; '; throw "PowerShell parser errors in ${Path}: $messages" }
}
function Invoke-SelfTest {
    Assert-Source; Test-FixParse -Path $SourceFix
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File $SourceFix -SelfTest
...
function Invoke-Install {
    ...
    $run = '@echo off' + "`r`n" + 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%LOCALAPPDATA%\WindowsSafeRebootGuardian\fix.ps1" %*' + "`r`n"
    [IO.File]::WriteAllText($RunCmd, $run, [Text.Encoding]::ASCII)
    $max = '@echo off' + "`r`n" + 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%LOCALAPPDATA%\WindowsSafeRebootGuardian\fix.ps1" -MaximumProtection %*' + "`r`n"
