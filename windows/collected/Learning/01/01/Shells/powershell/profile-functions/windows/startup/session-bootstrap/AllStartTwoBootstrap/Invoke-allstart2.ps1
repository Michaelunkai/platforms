...
function Invoke-AllStartTwoMain {
    ...
    try {
        ...
        if ($Mode -eq 'Startup') {
            Test-AllStartTwoLauncherArtifacts -Context $context
            ...
            Invoke-AllStartTwoImmediateLaunch -Context $context
        } else {
...
function Test-AllStartTwoLauncherArtifacts {
    param([pscustomobject]$Context)

    $requiredFiles = @(
        'C:\Users\micha\.claude\scripts\Start-TrayQuietApp.ps1',
        'C:\Users\micha\.claude\scripts\TrayQuietWin32.dll',
        'C:\Users\micha\.claude\scripts\trayquiet-start.vbs',
        'C:\Users\micha\.claude\scripts\openspeedy-silent.vbs'
    )
    foreach ($path in $requiredFiles) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Required hidden tray startup launcher is missing: $path"
        }
    }
    $murmureExe = 'C:\Program Files\murmure\murmure.exe'
    $murmureLauncher = 'C:\Users\micha\.claude\scripts\murmure-silent.vbs'
    if ((Test-Path -LiteralPath $murmureExe -PathType Leaf) -and (Test-Path -LiteralPath $murmureLauncher -PathType Leaf)) {
        Write-AllStartTwoLog $Context 'VERIFY' "Optional Murmure tray launcher is available: $murmureLauncher"
    } elseif (Test-Path -LiteralPath $murmureExe -PathType Leaf) {
        Write-AllStartTwoLog $Context 'WARN' "Murmure is installed but its optional tray launcher is absent; skipping it: $murmureLauncher"
    } else {
        Write-AllStartTwoLog $Context 'WARN' "Murmure is not installed; skipping its optional tray launcher check: $murmureExe"
    }
    Write-AllStartTwoLog $Context 'VERIFY' 'Hidden tray-quiet launcher artifact chain is present'
}
...
function Resolve-AllStartTwoCurrentWindowsAppsTargetPath {
...
function Resolve-AllStartTwoCurrentTargetPath {
    param([string]$TargetPath)
    ...
    $windowsAppsTarget = Resolve-AllStartTwoCurrentWindowsAppsTargetPath -TargetPath $expanded
...
function New-AllStartTwoCustomScheduledTaskSpec {
    ...
    $targetPath = Resolve-AllStartTwoCurrentTargetPath -TargetPath ([string]$Task.TargetPath)
...
function Invoke-AllStartTwoImmediateLaunch {
    param([pscustomobject]$Context)

    foreach ($taskSpec in @($Context.RequiredScheduledTasks)) {
        Start-AllStartTwoRequiredScheduledTask -Context $Context -TaskSpec $taskSpec
    }
    foreach ($runSpec in @($Context.RequiredRegistryRun)) {
        Start-AllStartTwoRequiredRunEntry -Context $Context -RunSpec $runSpec
    }
}

function Start-AllStartTwoRequiredScheduledTask {
    param(
        [pscustomobject]$Context,
        [hashtable]$TaskSpec
    )

    $taskName = [string]$TaskSpec.Name
    if (-not $taskName) { return }
    if ($Context.DryRun) {
        Write-AllStartTwoLog $Context 'ACTION' "DryRun: would immediately start required scheduled task $taskName"
        return
    }
    try {
        Start-ScheduledTask -TaskName $taskName -ErrorAction Stop
        Write-AllStartTwoLog $Context 'ACTION' "Immediately started required scheduled task $taskName"
    } catch {
        Write-AllStartTwoLog $Context 'WARN' "Failed to immediately start required scheduled task $taskName`: $($_.Exception.Message)"
    }
}

function Start-AllStartTwoRequiredRunEntry {
    param(
        [pscustomobject]$Context,
        [hashtable]$RunSpec
    )
    ...
    $command = [string]$RunSpec.Command
    if (-not $command) { return }
    $target = Get-AllStartTwoRunCommandTarget -Command $command
    if (-not $target -or -not (Test-Path -LiteralPath $target -PathType Leaf)) {
        Write-AllStartTwoLog $Context 'WARN' "Required Run entry target missing for $name`: $command"
        return
    }
    if ($Context.DryRun) {
        Write-AllStartTwoLog $Context 'ACTION' "DryRun: would immediately start required Run entry $name"
        return
    }
    try {
        Start-Process -FilePath $target -WindowStyle Hidden | Out-Null
        Write-AllStartTwoLog $Context 'ACTION' "Immediately started required Run entry $name"
    } catch {
        Write-AllStartTwoLog $Context 'WARN' "Failed to immediately start required Run entry $name`: $($_.Exception.Message)"
    }
}

function Get-AllStartTwoRunCommandTarget {
    param([string]$Command)

    if ([string]::IsNullOrWhiteSpace($Command)) { return $null }
    $trimmed = $Command.Trim()
    if ($trimmed.StartsWith('"')) {
        $nextQuote = $trimmed.IndexOf('"', 1)
        if ($nextQuote -gt 1) { return $trimmed.Substring(1, $nextQuote - 1) }
    }
